...
class DBManager:
    ...
    async def get_user_telegram_id(self, telegram_id: int):
        query = "SELECT * FROM users WHERE telegram_id = $1"
        result = await self.fetch(query, telegram_id)  # Assuming self.fetch executes the query and fetches results
        return result
